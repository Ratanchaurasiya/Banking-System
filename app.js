...
function showSection(sectionId) {
    ...
    } else if (sectionId === 'activeLoans') {
        updateActiveLoansUI();
    } else if (sectionId === 'customerLoans') {
        updateCustomerLoansUI();
    } else if (sectionId === 'payBills') {
        onBillTypeChange();
    } else if (sectionId === 'cards') {
        loadCardSettingsUI();
    } else if (sectionId === 'auditTrail') {
        renderAuditTrail();
...
function injectAuditTrailUI() {
    ...
    if (staffMenu && !document.getElementById('auditTrailMenuItem')) {
        const auditMenuItem = document.createElement('div');
        auditMenuItem.id = 'auditTrailMenuItem';
        auditMenuItem.className = 'menu-item';
        auditMenuItem.dataset.actionCall = "showSection('auditTrail')";
        auditMenuItem.innerHTML = '<i class="fas fa-clipboard-list"></i> Audit Trail';
        staffMenu.appendChild(auditMenuItem);
    }

    if (contentArea && !document.getElementById('auditTrail')) {
        const auditSection = document.createElement('div');
        auditSection.id = 'auditTrail';
        auditSection.className = 'section';
        auditSection.innerHTML = `
            <h2><i class="fas fa-clipboard-list"></i> Audit Trail</h2>
            <div class="audit-toolbar">
                <p style="color: #64748b;">Review recent local banking actions and system events.</p>
                <button class="btn btn-danger" data-action-call="clearAuditTrail()">
                    <i class="fas fa-trash"></i> Clear Audit Trail
                </button>
            </div>
        ...
        `;
        contentArea.appendChild(auditSection);
    }
}
...
function quickSearchAccount() {
    ...
    document.getElementById('quickSearchResult').innerHTML = `
        <div class="search-results">
            ...
            <p><strong>Balance:</strong> Rs. ${account.balance.toLocaleString()}</p>
...
function createAccount() {
    ...
    document.getElementById('summaryContent').innerHTML = `
        ...
        <p><strong>Opening Balance:</strong> Rs. ${newAccount.balance.toLocaleString()}</p>
...
function unlockAccount(accountNumber) {
    const result = bank.unlockAccount(accountNumber);
    if (result.success) {
        const statusFilter = document.getElementById('accountsStatusFilter');
        if (statusFilter && statusFilter.value === 'locked') {
            statusFilter.value = 'all';
        }

        bank.updateAllAccountsTable();
        showMessage(result.message, 'success', 'accountsResult');
    } else {
        showMessage(result.message, 'error', 'accountsResult');
    }
}
...
function confirmDeleteAccount(accountNumber) {
    ...
    if (account.balance > 0) {
        showMessage('Cannot close account with positive balance! Please withdraw all funds first (Balance: Rs. ' + account.balance.toLocaleString() + ').', 'error', 'accountsResult');
        return;
    }
...
function exportAccounts() {
    ...
    showMessage('Accounts exported successfully!', 'success', 'accountsResult');
}
...
function printAccountSummary() {
    ...
    printWindow.document.write('<div class="header"><h2>HexChal Digital Bank</h2><p>Official Account Opening Receipt & Record</p></div>');
