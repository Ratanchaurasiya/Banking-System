...
function renderStatementReport(account, txs) {
    ...
    summaryStats.innerHTML = `
        ...
        <p><strong>Total Money In:</strong> <span style="color: var(--success-color); font-weight: bold;">+₹${totalDeposits.toLocaleString()}</span></p>
        <p><strong>Total Money Out:</strong> <span style="color: var(--accent-color); font-weight: bold;">-₹${totalWithdrawals.toLocaleString()}</span></p>
    `;
        ...
        tableBody.innerHTML = txs.map(t => {
            ...
            return `
                <tr>
                    ...
                    <td>${t.description}</td>
                    <td>${t.mode}</td>
                    <td style="font-weight: 600; color:${color};">${isIn ? '+' : '-'}₹${t.amount.toLocaleString()}</td>
                </tr>
            `;
        }).join('');
    }
...
function renderGlobalTransactionsReport() {
    ...
    summaryStats.innerHTML = `
        ...
        <p><strong>Total Deposits:</strong> ₹${totalDeposit.toLocaleString()}</p>
        <p><strong>Total Withdrawals/Bills:</strong> ₹${totalWithdrawal.toLocaleString()}</p>
        <p><strong>Total Transfers:</strong> ₹${(totalTransfer/2).toLocaleString()} (net)</p>
        ...
        tableBody.innerHTML = txs.map(t => {
            ...
            return `
                <tr>
                    ...
                    <td><span style="font-weight:bold; color:${color};">${t.type}</span></td>
                    <td style="font-weight: 600; color:${color};">${isOut ? '-' : '+'}₹${t.amount.toLocaleString()}</td>
                    <td>${t.mode}</td>
                    <td>${t.description}</td>
...
function renderLoansReport() {
    ...
    summaryStats.innerHTML = `
        ...
        <p><strong>Approved Applications:</strong> ${approvedCount}</p>
        <p><strong>Total Loan Value Requested:</strong> ₹${totalLoanVal.toLocaleString()}</p>
    `;
    ...
    reportChartInstance = new Chart(ctx, {
        ...
        data: {
            ...
            datasets: [{
                data: [pendingCount, approvedCount, loans.filter(l => l.status === 'Rejected').length],
    ...
    if (loans.length === 0) {
        ...
        tableBody.innerHTML = loans.map(l => {
            ...
            else if (l.status === 'Approved') badge = `<span style="padding: 3px 8px; border-radius: 12px; font-size:11px; font-weight:bold; background:#d4edda; color:#155724; border: 1px solid #c3e6cb;">Approved</span>`;
            ...
            return `
                <tr>
                    ...
                    <td>${l.loan_type}</td>
                    <td style="font-weight: 600;">₹${l.amount.toLocaleString()}</td>
...
function exportPDF() {
    ...
    doc.setFontSize(22);
    doc.text('SECURE BANKING SYSTEM', 15, 22);
