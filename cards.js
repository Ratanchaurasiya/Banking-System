...
function toggleCardBlock() {
    ...
    if (typeof logAudit === 'function') {
        logAudit('Card Status Changed', `Debit card for account ${account.account_number} was ${toggleChecked ? 'frozen' : 'unfrozen'}.`);
    }
...
function saveCardLimits() {
    ...
    bank.saveData();
    if (typeof logAudit === 'function') {
        logAudit('Card Limits Updated', `ATM limit set to Rs. ${atm.toLocaleString()} and online limit set to Rs. ${online.toLocaleString()} for account ${account.account_number}.`);
    }
